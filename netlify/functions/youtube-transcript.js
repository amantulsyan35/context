...
exports.handler = async (event) => {
  ...
  if (!videoId || !startTime || !endTime) {
    return resp(
      400,
      JSON.stringify({ error: 'Missing required parameters: videoId, startTime, endTime' }),
      'application/json'
    );
  }
  ...
  try {
    // ---------- 1) Enumerate tracks via timedtext list (decompressed) ----------
    const listXml = await safeFetch(
      `https://www.youtube.com/api/timedtext?type=list&v=${encodeURIComponent(videoId)}`
    );
    const tracks = parseTrackList(listXml); // [{lang, name, kind}]

    // Prefer EN > EN asr > any
    const pick =
    ...
    if (pick) {
      ...
      if (isDebug) {
        return resp(
          200,
          JSON.stringify(
            { source: 'timedtext', picked: pick, segments: segs.length, sample: segs.slice(0, 3), tracks },
            null,
            2
          ),
          'application/json'
    ...
    if (!Array.isArray(foundTracks) || foundTracks.length === 0) {
      if (isDebug) {
        ...
        return resp(
          200,
          JSON.stringify({ source: 'scrape', reason: 'no_caption_tracks', tracks_list_count: tracks.length, consent_hint: consent }, null, 2),
          'application/json'
        );
      }
      return resp(200, '', 'text/plain');
    ...
    if (!track?.baseUrl) {
      if (isDebug) {
        return resp(
          200,
          JSON.stringify({ source: 'scrape', reason: 'no_base_url', languages: foundTracks.map(t => ({ lang: t.languageCode, kind: t.kind })) }, null, 2),
          'application/json'
        );
      }
      return resp(200, '', 'text/plain');
    ...
    if (isDebug) {
      return resp(
        200,
        JSON.stringify({ source: 'scrape', usedTrack: { lang: track.languageCode, kind: track.kind }, segments: segs2.length, sample: segs2.slice(0, 3), tracks_list_count: tracks.length }, null, 2),
        'application/json'
      );
    }
...
function parseTrackList(xml) {
  // <track id="0" lang_code="en" lang_translated="English" name="English" kind="asr"/>
  const tracks = [];
  const re = /<track\b([^>]+?)\/>/g;
  let m;
